...
public class TenthExercise {
  ...
  for (int student = 0; student < newScores.length; student++) {
    for (int score = 0; score < newScores[student].length; score++) {
      newScores[student][score] = scores[student][score];
    }
  }
...
for (int student = 0; student < newScores.length; student++) {
    for (int score = 0; score < newScores[student].length; score++) {
      if (newScores[student][score] < 90) {
        newScores[student][score] += 2.0;
      }
    }
  }
  System.out.println(Arrays.deepToString(newScores));
